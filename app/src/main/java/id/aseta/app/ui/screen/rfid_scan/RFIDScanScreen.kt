...
fun RFIDScanScreen(
    ...
    fun getTitleByType(type: String): String {
        return when(type) {
            "relocation" -> "Scanning Tag Relocation"
            "inspection" -> "Scanning Tag Inspection"
            "corrective_maintenance" -> "Scanning Tag Corrective Maintenance"
            "mutation" -> "Scanning Tag Mutation"
            else -> "Scanning Tag"
        }
    }
...
//    var resultScan by remember { mutableStateOf<List<AssetDetail>?>(emptyList()) }
    ...
    // Modifikasi DisposableEffect untuk cleanup yang lebih baik
...
//        }
    ...
    // Modifikasi LaunchedEffect(isScanning) untuk mengatasi masalah scanning berulang dan lag
...
//                        println("power rfidScan: ${mReader?.power}")
//                        // Set callback untuk mReader dengan debouncing
//                        mReader?.setInventoryCallback { tag ->
//                            tag?.let {
//                                // Cek apakah masih dalam mode scanning
...
//                                        }
//
//                                        // **SOLUSI 2: Cek lagi apakah masih scanning sebelum beep dan search**
////                                        if (isScanning) {
//                                            // 🔊 Beep dengan null safety
//                                            toneGenerator?.startTone(ToneGenerator.TONE_PROP_BEEP, 100)
//
//                                            // Switch ke Main thread untuk UI operations dengan delay untuk mengurangi lag
//                                            withContext(Dispatchers.Main) {
...
//                                    val power = powerLevel.toInt()
                ...
                if (mSled == null) {
                    ...
                    if (!isRunning) {
                        ...
                        // Set callback untuk mSled dengan debouncing
                                ...
                                CoroutineScope(Dispatchers.IO).launch {
                                    try {
                                        ...
                                        if (isScanning) {
                                            ...
                                            // Switch ke Main thread untuk UI operations dengan delay
                                            withContext(Dispatchers.Main) {
                                                delay(50) // Tambah small delay untuk mengurangi lag
                                                if (isScanning) { // Cek sekali lagi
                                                    assetCategoryModel.searchAssetByRfid(context, epc)
            ...
            // **SOLUSI 2: Improved cleanup saat stop scanning**
            if (isRunning) {
                // Stop inventory terlebih dahulu
                if(typeHH == "1"){
                    mReader?.stopInventory()
                } else {
                    mSled?.stopInventory()
